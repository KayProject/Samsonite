...
export function NodeConfigContent({
  ...
  const renderDropdown = (field: string, label: string, options: string[], currentVal: string) => (
    <div className="space-y-2 relative" key={field}>
      <div className="flex justify-between items-center">
        ...
        <span className="text-[8px] text-[#F59E0B] tracking-widest uppercase">[{currentVal || 'SELECT'}]</span>
      </div>
      <div className="bg-[#0F172A] border border-white/10 p-3.5 flex justify-between items-center focus-within:border-[#F59E0B]/50 transition-colors relative">
        <input
          type="text"
          value={currentVal || ''}
      ...
      <AnimatePresence>
        {activeDropdown === field && (
          <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} className="absolute top-full left-0 w-full bg-[#1E293B] border border-white/10 z-50 shadow-2xl max-h-48 overflow-y-auto custom-scrollbar mt-1">
            {options.map(opt => (
              <div key={opt} onMouseDown={() => up(field, opt)} className="p-3.5 text-[10px] font-bold text-white hover:bg-[#F59E0B]/20 hover:text-[#F59E0B] cursor-pointer border-b border-white/5 uppercase tracking-widest">
                {opt}
              </div>
            ))}
          </motion.div>
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-white/5 flex justify-between items-center bg-[#0F172A] shrink-0">
        <span className="text-[9px] text-[#F59E0B] tracking-[0.3em] uppercase font-bold">{node.data.type}_MODULE</span>
        {onClose && <button onClick={onClose} className="text-white/40 hover:text-white"><XMarkIcon className="w-4 h-4" /></button>}
      </div>
      <div className="p-6 overflow-y-auto space-y-8 flex-1 custom-scrollbar">
        {/* Module ID */}
        <div className="bg-[#1E293B]/50 border border-[#F59E0B]/20 p-4 flex items-center gap-4 relative">
          <div className="absolute -top-[1px] -left-[1px] w-2 h-2 border-t border-l border-[#F59E0B]" />
          <div className="w-8 h-8 bg-[#F59E0B]/10 border border-[#F59E0B]/30 flex items-center justify-center shrink-0">
            <span className="text-[#F59E0B] text-[10px]">↔</span>
          </div>
          <div>
            <div className="text-[10px] font-bold text-[#F59E0B] uppercase mb-1">{node.data.type}_MODULE</div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-[#0F172A] border border-white/10 p-3.5 flex items-center focus-within:border-[#F59E0B]/50 transition-colors">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            {SLIPPAGE.map(s => (
              <button key={s} onClick={() => setSlippage(s)} className={`flex-1 py-2 text-[9px] font-bold tracking-widest transition-all ${slippage === s ? 'bg-[#F59E0B]/20 border border-[#F59E0B] text-[#F59E0B]' : 'bg-[#1E293B] border border-white/5 text-white/40 hover:text-white'}`}>
                {s}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#0A0A12] border border-white/10 p-5 font-mono text-[11px] leading-relaxed text-white/70 shadow-inner">
            <span className="text-[#F59E0B]">if</span> (liquidity_depth &lt; 100k) {'{\n'}
            <div className="pl-4">route = <span className="text-[#22C55E]">"BALANCER_V2"</span>;</div>
            {'}\n'}
            <span className="text-[#F59E0B]">else</span> {'{\n'}
      ...
      {/* Apply / Reset Actions */}
      <div className="p-6 bg-[#0F172A] border-t border-white/5 shrink-0 space-y-4">
        <button onClick={onClose} className="w-full bg-[#F59E0B] text-white py-4 font-syne font-black uppercase text-[11px] tracking-[0.2em] hover:bg-[#F59E0B]/80 transition-colors">Apply Changes</button>
